...
// C/C++ source code generated on : Mon Nov 13 19:11:39 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"TrustAI_merge\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(TrustAI_merge_M, (NULL));
        TrustAI_merge_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(TrustAI_merge_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(TrustAI_merge_M)
                == (NULL)) || rtmGetStopRequested(TrustAI_merge_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(TrustAI_merge_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
